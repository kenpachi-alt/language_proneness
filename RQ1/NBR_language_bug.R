...
log_transform = function(df, log1 = log) {
       data.frame(
             ...
             language_r = relevel(df$language, rev(levels(df$language))[1]),
             commits = df$commits
         )
}
...
nbr_fit = glm.nb(bcommits~lmax_commit_age+ltins+ldevs+lcommits+language, contrasts = list(language = Weighted_contrast_encoding(log_transform_df$language)), data=log_transform_df)

#fitting second NBR model 
nbr_fit_r = glm.nb(bcommits~lmax_commit_age+ltins+ldevs+lcommits+language_r, contrasts = list(language_r = Weighted_contrast_encoding(log_transform_df$language_r)), data=log_transform_df)

# combine them into single result table
result = combine_models(nbr_fit, nbr_fit_r, log_transform_df$language)
result
